...
RUN apt-get update \
 ...
 && pip3 install --no-cache-dir flask waitress markdown pygments pymdown-extensions \
...
COPY handout.md /app/handout.md
...
ENV TMPDIR=/tmp
CMD ["python3","-c","from waitress import serve; import server; serve(server.app, host='0.0.0.0', port=8081, threads=16)"]
